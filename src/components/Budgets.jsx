    ...
    <div className="budgets-container">
      ...
      {budgets.map((budget) => (
        <div
          key={budget.id}
          className="budgets"
          style={{ borderLeft: "3px solid " + budget.color }}
        >
          <div id="budget">
            <div id="budget-name">{budget.name}</div>
                ...
                style={{
                  width:
                    budget.amountUsed >= budget.amount
                      ? "100%"
                      : budget.amountUsed > 0
                      ? ((budget.amountUsed / budget.amount) * 100).toString() +
                        "%"
                      : "0%",
                  background:
                    budget.amountUsed / budget.amount > 0.5
                      ? budget.amountUsed >= budget.amount
                        ? "red"
                        : "yellow"
                      : "green",
      ...
      <button className="addAccount" onClick={() => setDisplay("addBudget")}>
        +
      </button>
    </div>
  );
